...
import numpy as np
from architecture.utils.config import test, n_sequences_stat, n_symbols, n_sequences
...
def scatterplot_TxTi(Tx, Ti, t, plot_dir_s):
    ...
    ax.text(Tx, .5, f'Tx={Tx}')

    my_text = fr'n_simbols={n_symbols}' + '\n' + fr'n_iterations={n_sequences}' + '\n' + fr'Tx={Tx}'
...
def scatterplot_RvsFY(test, C0r, C0fy):
    ...
    :param C0r: a list containing the values of counters C0 using sequences read from file
...
def counters_distribution_Tx(c, n_seq, n_iter, t):
    ...
    textstr = '\n'.join(
        (f'$mean={np.mean(c):.2f}$', f'$std={np.std(c):.2f}$', f'p={p:.3f}$', fr'$\chi^2/ndf={chi_square_red:.2f}$'))
