...
def counters_distribution(c, n_seq, n_iter, t):
    ...
    ax.text(0.73, 0.90, textstr, transform=ax.transAxes, fontsize=14,
            verticalalignment='top', bbox=props)
...
def counters_distribution_file_plot(f, n_symbols, n_sequences, n_iterations_c):
    ...
    my_text += fr'n_simbols={n_symbols}' + '\n' + fr'n_sequences={n_sequences}' + '\n' \
               + fr'n_counters={n_iterations_c}' + '\n' + fr'test={test}' + '\n' \
               + fr'date={datetime.date.today().strftime("%d/%m/%Y")}' + fr'x_range=[{min(c)}] - [{max(c)}]'
    props = dict(boxstyle='square', facecolor='grey', alpha=0.15)  # bbox features
