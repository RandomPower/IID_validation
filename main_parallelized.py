...
def count_symbol_occurrences(input_file: str) -> tuple[dict[int, int], int]:
    ...
    n_chunks = (file_len // utils.read.DEFAULT_CHUNK_LEN) + 1

    for chunk in tqdm(utils.read.read_file_chunks(input_file), total=n_chunks, desc="Reading input file in chunks"):
        S = utils.read.symbols_from_bytes(chunk)
...
def min_entropy_function(conf: utils.config.Config) -> None:
    ...
    utils.plot.min_entropy(symbols_occ, frequencies, n_symbols, H_min, H_min_sigma, H_min_NIST)
    logger.debug("Save symbols occurrences and min-entropy values")
    utils.save.save_entropy(conf.input_file, symbols_occ, n_symbols, H_min, H_min_sigma, H_min_NIST)
